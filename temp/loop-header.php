...
?>
        ...
        <?php if ( is_tag() || is_category() || is_tax() ) { ?>
            ...
            <header class="loop-header tax-header">
                <h1 class="h loop-title tax-title"><a href=""><?php single_term_title(); ?></a></h1>
        ...
        <?php } elseif ( is_author() ) { ?>
            ...
            <header class="loop-header user-header vcard"><!-- microformats.org/wiki/hcard -->
                <h1 class="h loop-title user-title fn n"><?php the_author_meta( 'display_name' ); ?></h1>
        ...
        <?php } elseif ( is_date() ) { ?>
            ...
            <header class="loop-header date-header">
                <h1 class="h loop-title date-title"><?php echo implode('-', array_filter( explode( '/', $_SERVER['REQUEST_URI'] ), 'is_numeric' ) ); ?></h1>
        ...
        <?php } elseif ( is_search() ) { ?>
            ...
            <header class="loop-header search-header">
                <h1 class="h loop-title search-title"><?php echo ( __('Search: ', 'theme') . get_search_query() ); ?></h1>
            </header>

        <?php } elseif ( is_post_type_archive() ) { ?>
        
            <header class="loop-header">
                <h1 class="h loop-title"><?php post_type_archive_title(); ?></h1>
